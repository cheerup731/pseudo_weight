...
for (m in 1:M) {
  ...
  f.norm <- N * (1/includNP[Sstar]) / sum(1/includNP[Sstar])
...
capture.output(round(table, 4), file = "//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/var_ind_trueweight.txt")

total.time = Sys.time() - start_time

save.image("//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/var_ind_trueweight.RData")
