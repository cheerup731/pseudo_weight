...
capture.output(round(table, 4), file = "//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/var_ind_D.txt")

total.time = Sys.time() - start_time

save.image("//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/var_ind_D.RData")


#load("//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/var_ind_D.RData")
library("xtable")
table = as.data.frame(table)
table$RB = 100*(table$est-table$sim.var)/table$sim.var
table$CI = 100*table$CI
xtable(table[,c(6,5)], digits = c(0,2,2), caption = 'Variance Estimates of Ind_D')
