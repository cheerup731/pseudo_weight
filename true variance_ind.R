...
for (m in 1:M) {
  ...
  bias = (c(sum(sample.NP$y*f18)/sum(f18))- mean(pop$y))/M + bias
  mse = (c(sum(sample.NP$y*f18)/sum(f18)) - mean(pop$y))^2/M + mse
...
colnames(result) = c('n.NP', 'n.P', 'true_var_ind')

capture.output(round(result, 4), file = "//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/true_var_ind.txt")

total.time = Sys.time() - start_time

save.image("//cbsp.nl/Productie/secundair/MPOnderzoek/Werk/Combineren/Projecten/nonprobability/variance/true_var_ind.RData")
